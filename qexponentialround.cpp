...
qExponentialRound::qExponentialRound(double lambda, double q, unsigned long int xmin, unsigned long int N):lambda(lambda), q(q), xmin(xmin) {
    this->xmax = N - 1;
...
double qExponentialRound::F(double x) {
    double result = 1 - (1- q) * this->lambda * x;
    return result;
}

unsigned long int qExponentialRound::calc_x(double u) {
    unsigned long int x;
...
unsigned long int qExponentialRound::randint(){
    Uniform u = Uniform();
    double var = u.random();
    return calc_x(var);
}
